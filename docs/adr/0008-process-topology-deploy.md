...
| `DOCS_ENABLED` | `true` | gates operator-authenticated `GET /openapi.json` (issue #107) and `GET /openapi.yaml` (issue #108); `false` answers `404` with `code: "docs_disabled"` rather than deregistering the route |
